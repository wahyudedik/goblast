...
- [x] 1. Add gateway protection configuration to Laravel
  - [x] 1.1 Add `gateway_protection` key to `config/wa-automation.php`
  ...
  - [x] 1.2 Add corresponding environment variables to `.env.example`
...
- [x] 2. Implement device creation rate limiting in Laravel
  - [x] 2.1 Register `device-creation` rate limiter in `AppServiceProvider::boot()`
  ...
  - [x] 2.2 Apply `throttle:device-creation` middleware to `DeviceController::store`
  ...
  - [x] 2.3 Add `rateLimitStatus` AJAX endpoint to `DeviceController`
  ...
  - [x] 2.4 Write feature tests for device creation rate limiting
...
- [x] 3. Checkpoint — Ensure all Laravel tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Implement rate limit UI feedback with Alpine.js
  - [x] 4.1 Update `resources/views/devices/index.blade.php` with rate limit countdown
  ...
  - [x] 4.2 Handle direct route bypass for rate-limited users
    - In `DeviceController::create`, check rate limit status and redirect back with error if limited
    - _Requirements: 4.6_

  - [x] 4.3 Write feature test for rate limit UI bypass protection
...
- [x] 5. Implement BackoffManager in Node.js gateway
  - [x] 5.1 Create `gateway/src/backoffManager.js`
  ...
  - [x] 5.2 Implement backoff state persistence in `BackoffManager`
  ...
  - [x] 5.3 Write property tests for BackoffManager (install fast-check as dev dependency)
    - **Property 3: Exponential backoff delay within bounds**
    - **Validates: Requirements 3.1, 3.7**

  - [x] 5.4 Write property test: success resets backoff state
    - **Property 4: Success resets backoff state**
    - **Validates: Requirements 3.3**

  - [x] 5.5 Write property test: max retries triggers manual intervention
    - **Property 5: Max retries triggers manual intervention**
    - **Validates: Requirements 3.4**

  - [x] 5.6 Write property test: backoff state serialization round-trip
    - **Property 6: Backoff state serialization round-trip**
    - **Validates: Requirements 3.6**

  - [x] 5.7 Write property test: invalid configuration fallback to defaults
    - **Property 7: Invalid configuration fallback to defaults**
    - **Validates: Requirements 6.5**

- [x] 6. Implement SessionRestoreQueue in Node.js gateway
  - [x] 6.1 Create `gateway/src/sessionRestoreQueue.js`
  ...
  - [x] 6.2 Write property test: concurrent restoration limit
    - **Property 1: Concurrent restoration limit**
    - **Validates: Requirements 2.3**

  - [x] 6.3 Write property test: restoration fault tolerance
    - **Property 2: Restoration fault tolerance**
    - **Validates: Requirements 2.5**

- [x] 7. Integrate new components into DeviceManager and gateway startup
  - [x] 7.1 Update `gateway/src/deviceManager.js` to use `BackoffManager`
  ...
  - [x] 7.2 Update `gateway/src/deviceManager.js` to use `SessionRestoreQueue`
  ...
  - [x] 7.3 Add new webhook event methods to `gateway/src/webhookSender.js`
  ...
  - [x] 7.4 Add gateway environment variables to `gateway/.env` (or `.env.example`)
...
- [x] 8. Enhance gateway health endpoint
  - [x] 8.1 Update `/health` endpoint in `gateway/src/index.js`
  ...
  - [x] 8.2 Write unit tests for health endpoint response shape
    - Test response includes protection fields in idle, in_progress, and completed states
    - _Requirements: 5.1, 5.2, 5.4, 5.5_

- [x] 9. Handle new webhook events in Laravel
  - [x] 9.1 Update `ProcessWebhookJob` to handle new event types
  ...
  - [x] 9.2 Write feature tests for new webhook event handling
...
- [x] 11. Final wiring and validation
  - [x] 11.1 Verify route registration completeness
  ...
  - [x] 11.2 Run `vendor/bin/pint --dirty --format agent` on all modified PHP files
    - _Requirements: Code style compliance_

  - [x] 11.3 Write integration test for full device creation flow with rate limiting
