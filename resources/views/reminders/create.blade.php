...
@section('page-title', 'Buat Reminder')
...
@section('content')
    ...
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-900">Buat Reminder Baru</h2>
        <p class="mt-1 text-sm text-gray-600">Buat pengingat otomatis yang berulang — seperti alarm untuk mengirim pesan
            WhatsApp.</p>
    </div>
        ...
        <form action="{{ route('reminders.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="spp_due" {{ old('type') === 'spp_due' ? 'selected' : '' }}>⏰ SPP Jatuh Tempo</option>
                        <option value="invoice_unpaid" {{ old('type') === 'invoice_unpaid' ? 'selected' : '' }}>📄 Invoice
                            Belum Bayar</option>
                        <option value="booking_tomorrow" {{ old('type') === 'booking_tomorrow' ? 'selected' : '' }}>📅
                ...
                <div>
                    ...
                    <select name="frequency" id="frequency" required
                        ...
                        <option value="weekly" {{ old('frequency') === 'weekly' ? 'selected' : '' }}>📅 Setiap Minggu
                        </option>
                        <option value="monthly" {{ old('frequency') === 'monthly' ? 'selected' : '' }}>🗓️ Setiap Bulan
                        </option>
                        <option value="yearly" {{ old('frequency') === 'yearly' ? 'selected' : '' }}>📆 Setiap Tahun
                        </option>
                    </select>
                    @error('frequency')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="send_time" class="block text-sm font-semibold text-gray-900 mb-2">Jam Kirim <span
                            class="text-red-500">*</span></label>
                    <input type="time" name="send_time" id="send_time" value="{{ old('send_time', '08:00') }}" required
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
                ...
                <button type="submit" {{ $devices->isEmpty() ? 'disabled' : '' }}
                    class="px-6 py-2.5 bg-green-600 border border-transparent rounded-lg font-semibold text-sm text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-150 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                    Buat Reminder
                </button>
            </div>
        </form>
